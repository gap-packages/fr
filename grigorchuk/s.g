...
F := GF(p);
G := FreeGroup("a","b","c","d");
g1 := Identity(G); ga := G.1; gb := G.2; gc := G.3; gd := G.4;
...
R := GroupRing(F,G);
one := One(R); a := R.2; b := R.4; c := R.6; d := R.8;
A := a+one; B := b+one; C := c+one; D := d+one;
...
ReduceWord := function(x)
  ...
  repeat
    ...
    od;
  until not changed;
  return ObjByExtRep(FamilyObj(x), e);
end;
...
### UGLY! we're probably stuck in some sort of recursion
  if Length(Support(t))>499 then Error("I'm probably stuck!"); fi;
